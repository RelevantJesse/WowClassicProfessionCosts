...
This repo contains **ProfessionLeveler** (Blazor Server app) and **ProfessionLevelerScan** (in-game addon) to generate profession leveling plans using real-ish time auction pricing.

## Where we're at

### Web app
- **Architecture:** `ProfessionLeveler.Core` (domain + planner), `ProfessionLeveler.Infrastructure` (SQLite EF Core + providers + data packs), `ProfessionLeveler.Web` (Blazor UI + minimal APIs), `ProfessionLeveler.Tests`.
...
2) Go to `/targets` and click **Install targets** (writes `ProfessionLevelerScan_Targets.lua` into the addon folder).
